...
- `买单被吃单后，立即以 0.01 紧急卖出`：默认关闭。开启后，买单被吃单（包括部分成交）时，机器人会撤掉该市场其余订单、停止继续报价，并以 `0.01` 的非 post-only 卖单卖出已成交数量。这可能会造成损失，只应在你明确接受这一结果时开启。
...
- `dry_run`：是否处于模拟运行。
- `emergency_exit_on_buy_fill`：买单被吃单后是否以 `0.01` 紧急卖出；默认 `false`。
