...
收到买入撮合通知时，机器人会提前准备卖单签名，并在请求额度允许时快速核对这张买单的实际成交量。订单接口先确认成交时，无需等 WebSocket 的结算成功通知即可开始卖出；只有撮合通知、尚未核对到实际成交量时，不会盲目卖出账户原有持仓。新增快速核对最多全账户每秒一次、每次撮合持续最多 10 秒，之后仍由常规成交监测兜底。卖单明确因份额不足被拒绝时会短间隔重试；遇到限流会退避，超时或结果不明确时只核对原订单，避免重复卖出。此功能不能消除链上结算等待，也不保证成交速度或卖出价格。
