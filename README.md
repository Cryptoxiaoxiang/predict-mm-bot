...
- `买单被吃单后，立即以 0.01 紧急卖出`：默认勾选。检测到成交结算已提交时，机器人会立即撤掉该市场其余订单并停止继续报价；链上结算成功、买到的份额到账后，再以 `0.01` 的非 post-only 卖单卖出已成交数量。如果 Predict.fun 的可用份额同步稍有延迟，机器人会自动重试卖出，不会因为一次“份额不足”错误而停止。这可能会造成损失。
