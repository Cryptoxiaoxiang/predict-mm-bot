...
def test_approached_custom_no_buy_uses_canonical_outcome_side() -> None:
    ...
    engine.open_orders["old-hrts-buy"] = ManagedOrder(
        order_id="old-hrts-buy",
        quote=Quote(
            "818058",
            Side.BUY,
            Decimal("0.93"),
    ...
    book = OrderBook(
        ...
        asks=[Level(Decimal("0.06"), Decimal("100"))],
        tick_size=Decimal("0.01"),
    )

    asyncio.run(engine._cancel_orders_approached_by_market("818058", book))

    assert client.cancelled == ["old-hrts-buy"]
